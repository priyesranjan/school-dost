...
router.get('/entries', requireAuth, async (req, res) => {
  try {
    const data = await listTimetableEntries(req.tenantDb!, {
      ...
      day: req.query.day
        ? (String(req.query.day) as 'Monday' | 'Tuesday' | 'Wednesday' | 'Thursday' | 'Friday' | 'Saturday')
        : undefined,
    })
    res.json({ data })
  } catch (error) {
...
router.post(
  ...
  writeActionLimiter,
  validateBody(timetableCreateSchema),
  async (req, res) => {
    ...
    try {
      const result = await createTimetableEntry(req.tenantDb!, {

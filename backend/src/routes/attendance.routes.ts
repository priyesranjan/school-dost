...
router.get('/', requireAuth, requireRole(['admin', 'teacher', 'hod', 'receptionist', 'parent', 'student']), async (req, res) => {
  try {
    const data = await listAttendance(req.tenantDb!, {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const result = await createAttendance(req.tenantDb!, {
...
router.post(
  ...
  requireRole(['admin', 'teacher']),
  writeActionLimiter,
  validateBody(attendanceBulkCreateSchema),
  async (req, res) => {
    const body = req.body as { records: { student_id: number; date: string; status: 'present' | 'absent' | 'late' }[] }
    try {
      const data = await bulkCreateAttendance(req.tenantDb!, body.records)
      await appendAuditLog(req.tenantDb!, {

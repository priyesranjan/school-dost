...
function parsePerPage(v: unknown, fb: number) { const n = Number(v); if (!Number.isFinite(n) || n <= 0) return fb; return Math.min(100, Math.floor(n)) }
...
router.post('/', requireAuth, requireRole(['admin', 'teacher']), writeActionLimiter, validateBody(examCreateSchema), async (req, res) => {
  const body = req.body as { name: string; class_name: string; subject: string; date: string; max_marks: number; academic_year: string }
  try {
    const data = await createExam({ ...body, name: body.name.trim(), class_name: body.class_name.trim(), subject: body.subject.trim(), academic_year: body.academic_year.trim() })
    await appendAuditLog({ action: 'exam_created', module: 'fees', actor_name: req.auth?.name || 'Unknown', actor_role: req.auth?.role || 'unknown', target: data.name, metadata: `class=${data.class_name} subject=${data.subject}` })
    res.json({ data })
  } catch (error) {
...
router.delete('/:id', requireAuth, requireRole(['admin']), writeActionLimiter, async (req, res) => {
  ...
  if (!Number.isFinite(id) || id <= 0) { res.status(400).json({ error: { code: 'INVALID_REQUEST', message: 'Invalid exam id' } }); return }
  try {
    await deleteExam(id)
    await appendAuditLog({ action: 'exam_deleted', module: 'fees', actor_name: req.auth?.name || 'Unknown', actor_role: req.auth?.role || 'unknown', target: `exam_id=${id}`, metadata: '' })
...
router.get('/:id/results', requireAuth, async (req, res) => {
  ...
  if (!Number.isFinite(id) || id <= 0) { res.status(400).json({ error: { code: 'INVALID_REQUEST', message: 'Invalid exam id' } }); return }
  try {
    const results = await listExamResults(id)
...
router.post('/results', requireAuth, requireRole(['admin', 'teacher']), writeActionLimiter, validateBody(examResultUpsertSchema), async (req, res) => {
  const body = req.body as { exam_id: number; student_id: number; marks_obtained: number }
  try {
    const data = await upsertExamResult(body)
    await appendAuditLog({ action: 'exam_result_recorded', module: 'fees', actor_name: req.auth?.name || 'Unknown', actor_role: req.auth?.role || 'unknown', target: `exam_id=${body.exam_id} student_id=${body.student_id}`, metadata: `marks=${data.marks_obtained} grade=${data.grade}` })
    res.json({ data })
  } catch (error) {

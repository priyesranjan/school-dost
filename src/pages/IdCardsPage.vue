  ...
  <div class="mx-auto max-w-7xl space-y-10 animate-fade-in-up print:m-0 print:p-0">
    ...
    <div class="space-y-10 print:hidden">
      ...
      <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div
          class="flex flex-wrap items-center gap-4 bg-white p-2 rounded-3xl shadow-xl border border-gray-100 dark:bg-gray-800 dark:border-gray-700"
        >
          <div class="flex items-center gap-4 px-4 py-2 border-r border-gray-100 dark:border-gray-700">
            <div class="flex flex-col">
              ...
              <select
                v-model="classFilter"
                class="bg-transparent text-sm font-black text-gray-900 dark:text-white focus:outline-none cursor-pointer"
              >
          ...
          <div class="flex gap-4 px-2">
            <AppButton
              variant="secondary"
              size="sm"
              class="rounded-2xl border-none bg-gray-50 text-gray-600 hover:bg-gray-100"
              @click="toggleAll"
            >
            ...
            >
              Preview {{ issueMode === 'id' ? 'IDs' : 'Passes' }} ({{ selectedIds.length }})
            </AppButton>
          </div>
        </div>
      </div>
      ...
      <AppCard title="Institutional Registry" :no-padding="true" class="border-none shadow-2xl overflow-hidden">
          ...
          <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
              ...
              >
                  ...
                  <div class="flex items-center gap-4 cursor-pointer" @click="openPreview(s)">
                    <div class="h-10 w-10 relative">
                      <img
                        ...
                        alt=""
                        class="h-full w-full rounded-2xl object-cover shadow-sm group-hover:scale-110 transition-transform"
                      />
                      <div
                        v-else
                        class="flex h-full w-full items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600 text-xs font-black"
                      >
                        {{ s.name.charAt(0) }}
                      </div>
                      <div
                        class="absolute -bottom-1 -right-1 h-3 w-3 bg-emerald-500 rounded-full border-2 border-white"
                      ></div>
                    </div>
                    <div>
                ...
                <td class="px-8 py-4">
                  <span
                    class="px-3 py-1 bg-gray-100 dark:bg-gray-700/50 rounded-lg text-[10px] font-mono font-black text-gray-600 dark:text-gray-400"
                  >
                    REF:{{ s.roll_number }}
                  </span>
                </td>
                ...
                <td class="px-8 py-4 text-right">
                  <AppButton
                    variant="secondary"
                    size="xs"
                    class="rounded-xl border-none bg-indigo-50 text-indigo-600 hover:bg-indigo-100 px-4"
                    @click="openPreview(s)"
                  >
                    Visualize
                  </AppButton>
        ...
        <EmptyState
          ...
          title="No members detected"
          message="Adjust your identity filters to visualize records."
        />
      </AppCard>
      ...
      <AppModal v-model="previewActive" :title="`Identity Visualization: ${previewStudent?.name}`" size="md">
        <div
          class="flex flex-col items-center py-10 space-y-10 bg-gray-50/50 dark:bg-gray-900/10 rounded-3xl border border-gray-100 dark:border-gray-800"
        >
          <div
            class="flex gap-4 bg-white dark:bg-gray-800 p-1.5 rounded-2xl shadow-inner border border-gray-100 dark:border-gray-700 mb-4 transition-all"
          >
            <button
              @click="issueMode = 'id'"
              :class="[
                'px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all',
                issueMode === 'id' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50',
              ]"
            >
              Member ID
            </button>
            <button
              ...
              :class="[
                'px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all',
                issueMode === 'bus' ? 'bg-amber-500 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50',
              ]"
            >
              Bus Pass
            </button>
          ...
          <div class="transition-all transform hover:scale-[1.02]">
            ...
            <BusPass
              v-if="previewStudent && issueMode === 'bus'"
              :student="previewStudent"
              routeNo="A-12"
              stopName="Central Plaza Hub"
            />
          </div>
          ...
          <div class="flex gap-4">
            <AppButton
              ...
              class="rounded-2xl border-none font-black text-xs px-8 h-12 uppercase tracking-widest"
            >
              Close
            </AppButton>
            <AppButton
              @click="printSingle"
              class="rounded-2xl font-black text-xs px-10 h-12 uppercase tracking-widest shadow-2xl"
              :class="issueMode === 'id' ? 'bg-indigo-600' : 'bg-amber-500 shadow-amber-100'"
            >
        ...
        <template #footer>
          <div class="flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
            <div
              :class="['h-2 w-2 rounded-full animate-pulse', issueMode === 'id' ? 'bg-indigo-500' : 'bg-amber-500']"
            ></div>
      ...
      <!-- Bulk Preview Modal -->
      <AppModal v-model="bulkPreviewActive" title="Bulk Identity Visualization" size="lg">
        <div
          class="flex flex-col items-center py-6 bg-gray-50/50 dark:bg-gray-900/10 rounded-3xl border border-gray-100 dark:border-gray-800"
        >
          <div
            class="flex gap-4 bg-white dark:bg-gray-800 p-1.5 rounded-2xl shadow-inner border border-gray-100 dark:border-gray-700 mb-6 transition-all"
          >
            <button
              @click="issueMode = 'id'"
              :class="[
          ...
          <div
            class="max-h-[50vh] w-full overflow-y-auto p-4 grid grid-cols-1 sm:grid-cols-2 gap-8 place-items-center bg-gray-100 dark:bg-gray-900 rounded-xl inner-shadow"
          >
            <template v-if="issueMode === 'id'">
              <IdCard
                v-for="s in selectedStudentsForPrint"
                :key="s.id"
                :student="s"
                class="shadow-xl transform transition-all hover:scale-[1.02]"
              />
            </template>
            <template v-if="issueMode === 'bus'">
              <BusPass
                v-for="s in selectedStudentsForPrint"
                :key="s.id"
                :student="s"
    ...
    <!-- ALWAYS IN DOM BUT INVISIBLE ON SCREEN TO PRELOAD IMAGES AND FIX BROWSER TIMING -->
    <Teleport to="body">
      <div
        id="id-print-area"
      ...
      >
        <div v-if="issueMode === 'id'" class="flex flex-wrap gap-8 justify-center items-start pt-8 pb-8">
          <div
            v-for="s in selectedStudentsForPrint"
            :key="s.id"
            class="w-[45%] flex justify-center items-center break-inside-avoid shadow-none border-none mb-4"
          >
            <IdCard :student="s" />
          </div>
        </div>
        <div v-if="issueMode === 'bus'" class="flex flex-wrap gap-8 justify-center items-start pt-8 pb-8">
          <div
            v-for="s in selectedStudentsForPrint"
            :key="s.id"
            class="w-[45%] flex justify-center items-center break-inside-avoid shadow-none border-none mb-4"
...
const bulkPreviewActive = ref(false)
...
const selectedStudentsForPrint = computed(() => {
  if (printIds.value.length > 0) {
    return studentStore.students.filter((s) => printIds.value.includes(s.id))
  }
  return studentStore.students.filter((s) => selectedIds.value.includes(s.id))
})
...
async function triggerPrint(ids: number[]) {
  ...
  // Wait explicitly to ensure Vue evaluates reactivity and DOM is updated
  await nextTick()
  await new Promise((resolve) => setTimeout(resolve, 500))

  // Natively trigger print dialog
  window.print()

  // Reset print target so it falls back to normal preview mode selection
  printIds.value = []
...
function openBulkPreview() {
  if (!selectedIds.value.length) {
    toast.warning('Member selection required for preview.')
    return
  }
  bulkPreviewActive.value = true
}
</script>

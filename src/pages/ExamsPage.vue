  ...
  <div class="space-y-8 animate-fade-in-up">
    ...
    >
      <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div
            class="inline-flex items-center gap-2 rounded-full bg-purple-50 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-purple-600 dark:bg-purple-900/30"
          >
            📊 Examination Portal
          </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <AppButton
            ...
            class="shadow-lg shadow-primary-200 dark:shadow-none"
          >
            + Schedule New Exam
          </AppButton>
        </div>
      ...
      <!-- Background Accents -->
      <div
        class="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-purple-50/50 blur-3xl dark:bg-purple-900/10"
      ></div>
    </div>
    ...
    <Transition name="fade" mode="out-in">
      <div :key="activeTab">
          ...
          <AppCard :no-padding="true" class="overflow-hidden shadow-2xl">
            <div class="overflow-x-auto">
              <table class="w-full text-left text-sm">
                <thead>
                  <tr
                    class="bg-gray-50/30 text-[10px] font-black uppercase tracking-widest text-gray-400 dark:bg-gray-900/30"
                  >
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                  ...
                  >
                    <td class="px-8 py-5">
                      <div class="flex items-center gap-3">
                        <div
                          class="flex h-10 w-10 items-center justify-center rounded-xl bg-purple-50 text-purple-600 shadow-inner dark:bg-purple-900/20"
                        >
                          📜
                        </div>
                    ...
                    <td class="px-8 py-5">
                      <span
                        class="rounded-lg bg-gray-100 px-2 py-0.5 text-[10px] font-black uppercase text-gray-500 dark:bg-gray-800"
                      >
                        {{ exam.class_name }}
                      </span>
                    </td>
                      ...
                      <div class="flex items-center gap-2">
                        <div class="h-1.5 w-16 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800/50">
                          <div
                            ...
                            :style="{
                              width: `${(examStore.getExamResults(exam.id).length / studentStore.students.length) * 100}%`,
                            }"
                          ></div>
                        </div>
                        <span class="text-[9px] font-black uppercase text-emerald-600"
                          >{{ examStore.getExamResults(exam.id).length }} Entries</span
                        >
                      </div>
                    </td>
                    <td class="px-8 py-5 text-right">
                      <div class="flex items-center justify-end gap-2">
                        <button
                          @click="openMarksEntry(exam)"
                          class="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 transition-all hover:scale-110 hover:bg-indigo-600 hover:text-white dark:bg-indigo-900/20"
                        ...
                        <button
                          ...
                          class="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600 transition-all hover:scale-110 hover:bg-emerald-600 hover:text-white dark:bg-emerald-900/20"
            ...
            <EmptyState
              v-if="!examStore.exams.length"
              title="Assessment Registry Empty"
              message="No pending exams scheduled for this term."
            />
          </AppCard>
        </template>
        ...
        <template v-if="activeTab === 'marks'">
          <div class="space-y-6">
            <div
              class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between bg-gray-50/50 p-6 rounded-3xl dark:bg-gray-800/30"
            >
              <div class="flex-1 max-w-md space-y-2">
                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400"
                  >Select Assessment Domain</label
                >
                <select
                  v-model="selectedExamId"
                  class="w-full rounded-2xl border border-gray-100 bg-white px-5 py-3 text-sm font-black text-gray-900 outline-none transition-all focus:ring-4 focus:ring-primary-50 dark:border-gray-700 dark:bg-gray-900 dark:text-white"
                >
              ...
              <AppButton
                v-if="selectedExamId && marksEntries.length"
                variant="primary"
                size="lg"
                shadow="xl"
                @click="saveMarks"
            ...
            <AppCard v-if="selectedExam" :no-padding="true" class="shadow-2xl">
              <div
                class="flex items-center justify-between px-8 py-6 border-b border-gray-50 bg-white dark:bg-gray-800"
              >
                <div>
                  <h3 class="text-lg font-black text-gray-900 dark:text-white">{{ selectedExam.name }}</h3>
                  <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                    {{ selectedExam.class_name }} · Scale: {{ selectedExam.max_marks }} Marks
                  </p>
                </div>
              ...
              <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                  ...
                  <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                    <tr
                      v-for="entry in marksEntries"
                      :key="entry.studentId"
                      class="group hover:bg-primary-50/30 transition-colors"
                    >
                      <td class="px-8 py-5">
                        <div class="flex items-center gap-4">
                          <span
                            class="flex h-10 w-10 items-center justify-center rounded-xl bg-gray-100 text-[10px] font-black text-gray-500"
                          >
                            #{{ entry.rollNumber }}
                          </span>
                          <div>
                      ...
                      <td class="px-8 py-5 text-center">
                        <div
                          :class="[
                            'inline-flex h-10 w-10 items-center justify-center rounded-xl text-xs font-black shadow-lg shadow-gray-100 dark:shadow-none',
                            gradeBackground(entry.grade),
                          ]"
                        >
                          {{ entry.grade }}
                        </div>
                        ...
                        <div class="flex items-center justify-center gap-1.5">
                          <span
                            ...
                            :class="[
                              ...
                              (entry.marks / selectedExam.max_marks) * 5 >= i
                                ? 'bg-emerald-500'
              ...
              <EmptyState
                v-if="!marksEntries.length"
                title="No Candidates"
                message="Select an active assessment to begin performance entry."
                class="py-12"
              />
            </AppCard>
          </div>
        ...
        <template v-if="activeTab === 'results'">
          <div class="space-y-8">
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <div
                v-for="avg in examStore.classAverage"
                :key="avg.examId"
              ...
              >
                <div class="flex items-center justify-between mb-4">
                  <div
                    class="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-50 text-xl dark:bg-indigo-900/20"
                  >
                    📈
                  </div>
                  <div class="text-right">
                    ...
                    <p
                      :class="[
                        ...
                        avg.average >= 60 ? 'text-emerald-600' : 'text-rose-600',
                      ]"
                    >
                      {{ avg.average }}%
                    </p>
            ...
            <AppCard title="Performance Ledger" :no-padding="true" class="shadow-2xl">
              <!-- Filter Bar -->
              <div
                class="flex flex-wrap items-center gap-4 border-b border-gray-50 p-6 bg-gray-50/30 dark:border-gray-700 dark:bg-gray-900/30"
              >
                <select
                  v-model="resultExamFilter"
                  class="rounded-xl border border-gray-100 bg-white px-4 py-2 text-[10px] font-black uppercase tracking-widest text-gray-900 outline-none hover:border-primary-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                >
                  <option value="">All Assessment Cycles</option>
                  <option v-for="name in examStore.examNames" :key="name" :value="name">{{ name }}</option>
                </select>
                <select
                  v-model="resultSubjectFilter"
                  class="rounded-xl border border-gray-100 bg-white px-4 py-2 text-[10px] font-black uppercase tracking-widest text-gray-900 outline-none hover:border-primary-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                >
                  ...
                  <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                    <tr v-for="r in filteredResults" :key="r.id" class="group hover:bg-emerald-50/20 transition-colors">
                      <td class="px-8 py-5">
                        <div class="flex items-center gap-4">
                          <div
                            class="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-primary-400 to-indigo-500 text-xs font-black text-white shadow-lg"
                          >
                      ...
                      <td class="px-8 py-5 text-xs font-black text-gray-900 dark:text-white uppercase tracking-tighter">
                        {{ r.subject }}
                      </td>
                      <td class="px-8 py-5 text-right font-black text-indigo-600">
                        {{ r.marks_obtained }}<span class="text-gray-300 mx-1">/</span>{{ r.max_marks }}
                      </td>
                      <td class="px-8 py-5">
                        <div class="flex flex-col items-center gap-1">
                          <span
                            :class="[
                              'rounded-full px-2 py-0.5 text-[9px] font-black uppercase text-white shadow-md',
                              gradeBackground(r.grade),
                            ]"
                            >{{ r.grade }}</span
                          >
                          <span class="text-[9px] font-black text-gray-400"
                            >{{ Math.round((r.marks_obtained / r.max_marks) * 100) }}%</span
                          >
                        </div>
                      </td>
                    </tr>
              ...
              <EmptyState
                ...
                title="Data Visualization Silent"
                message="Complete score entry to unlock deep dive analytics."
                class="py-16"
              />
            </AppCard>
          </div>
        ...
        <template v-if="activeTab === 'admit'">
          <div class="space-y-6">
            ...
            >
              <div class="flex flex-wrap items-center gap-4">
                ...
                <div class="space-y-1">
                  ...
                  <p class="text-xs font-black text-indigo-600 uppercase">
                    {{ selectedPassIds.length }} Candidates Identified
                  </p>
                </div>
              </div>
              <div class="flex gap-3">
                <AppButton
                  variant="secondary"
                  size="sm"
                  class="rounded-xl border-none font-black"
                  @click="togglePassSelection"
                >
                ...
                <AppButton
                  variant="primary"
                  size="sm"
                  class="rounded-xl px-8 shadow-xl shadow-indigo-100"
                  @click="printAdmitCards"
                >
            ...
            <AppCard
              title="Candidate Eligibility Matrix"
              :no-padding="true"
              class="shadow-2xl overflow-hidden text-left"
            >
              <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                  <thead>
                    <tr
                      class="bg-gray-50/50 text-[10px] font-black uppercase tracking-widest text-gray-400 dark:bg-gray-900/30 text-left"
                    >
                  ...
                  <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                    <tr
                      v-for="s in filteredPassStudents"
                      :key="s.id"
                    ...
                    >
                      ...
                      <td class="px-8 py-5">
                        <span class="text-xs font-bold text-gray-500 uppercase tracking-tighter"
                          >{{ s.class_name }} · {{ s.section }}</span
                        >
                      </td>
                      <td class="px-8 py-5 font-mono text-xs text-gray-400 font-bold italic">{{ s.roll_number }}</td>
                      <td class="px-8 py-5 text-right">
                        <div
                          class="flex items-center justify-end gap-2 text-[10px] font-black text-emerald-600 uppercase tracking-widest"
                        >
              ...
              <EmptyState
                v-if="!filteredPassStudents.length"
                title="No candidates identified"
                message="Adjust your filters to reveal candidate records."
              />
            </AppCard>
          </div>
        </template>
        ...
        <template v-if="activeTab === 'marksheet'">
          <div class="space-y-6 text-center py-20 bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200">
            <div
              class="h-24 w-24 bg-white shadow-2xl rounded-3xl mx-auto flex items-center justify-center text-4xl mb-6"
            >
              📜
            </div>
            <h2 class="text-2xl font-black text-gray-900 tracking-tight">Academic Achievement Ledger</h2>
            <p class="max-w-md mx-auto text-gray-400 font-medium">
              Issue high-fidelity marksheets for completed assessment cycles. Verify results before finalizing the
              ledger.
            </p>

            <div class="mt-10 max-w-sm mx-auto space-y-4">
              <select
                v-model="reportClassFilter"
                class="w-full bg-white border border-gray-200 rounded-[1.5rem] px-6 py-4 font-black text-sm outline-none shadow-xl shadow-gray-100 dark:bg-gray-800 dark:border-gray-700"
              >
              ...
              <AppButton
                ...
                variant="primary"
                size="lg"
                class="w-full rounded-[1.5rem] py-8 text-lg shadow-2xl shadow-indigo-100"
                @click="printClassMarksheets"
              >
    ...
    <AppModal v-model="showExamModal" title="Schedule Assessment" size="md" class="dark:border dark:border-gray-800">
      ...
      <template #footer>
        <div class="flex items-center justify-end gap-4 p-2">
          <AppButton variant="secondary" @click="showExamModal = false" class="border-none hover:bg-gray-100"
            >Dismiss</AppButton
          >
          <AppButton @click="handleScheduleExam" class="px-8 shadow-xl shadow-primary-100"
            >Establish Schedule</AppButton
          >
        </div>
      </template>
    </AppModal>
...
function printClassMarksheets() {
  const ids = studentStore.students.filter((s) => s.class_name === reportClassFilter.value).map((s) => s.id)

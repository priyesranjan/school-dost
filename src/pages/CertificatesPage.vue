<template>
  <div class="mx-auto max-w-7xl space-y-10 animate-fade-in-up print:m-0 print:p-0">
    ...
    <div class="space-y-10 print:hidden">
      <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div class="flex items-center gap-4 bg-white p-2 rounded-3xl shadow-xl dark:bg-gray-800">
           ...
           <AppButton variant="primary" class="rounded-2xl px-8 shadow-xl shadow-primary-200" @click="showIssueModal = true">
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
         <StatCard title="Transfer Credits" :value="certificateStore.tcCertificates.length" icon="🏫" icon-bg="bg-amber-50 text-amber-600" />
         <StatCard title="Character Ratings" :value="certificateStore.characterCertificates.length" icon="⭐" icon-bg="bg-emerald-50 text-emerald-600" />
         <StatCard title="Pending Auth" :value="0" icon="⚖️" icon-bg="bg-indigo-50 text-indigo-600" />
      </div>

      <!-- Credential Ledger -->
      <AppCard title="Institutional Credential Ledger" :no-padding="true" class="border-none shadow-2xl overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
              <tr v-for="c in certificateStore.certificates" :key="c.id" class="group hover:bg-indigo-50/30 transition-colors">
                <td class="px-8 py-4">
                  <span class="px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded-lg text-[10px] font-mono font-black text-gray-600 dark:text-gray-400">
                    CERT:{{ c.certificate_no }}
                  </span>
                </td>
                <td class="px-8 py-4">
                  <div class="flex items-center gap-3">
                    <div class="h-10 w-10 bg-indigo-50 rounded-xl flex items-center justify-center font-black text-indigo-600 uppercase">
                ...
                <td class="px-8 py-4 text-right">
                  <AppButton size="xs" variant="secondary" class="rounded-xl border-none bg-indigo-50 text-indigo-600 hover:bg-indigo-100 px-6 font-black" @click="triggerCertPrint(c)">
                    Visualize & Output
                  </AppButton>
                </td>
              </tr>
        ...
        <EmptyState v-if="!certificateStore.certificates.length" title="Credential Ledger Silenced" message="Issue your first official institutional certificate today." />
      </AppCard>
    </div>

    <!-- BULLETPROOF PRINT ENGINE (TELEPORTED TO BODY AT ROOT) -->
    <Teleport to="body">
       <div v-if="isPrinting" id="cert-print-engine" class="bg-white !p-0 !m-0 fixed inset-0 z-[9999] overflow-y-scroll print:overflow-visible">
          <div v-if="printCert" class="flex flex-col items-center p-20 scale-[0.9]">
             ...
             >
                <template v-if="printCert.type === 'tc'">
                   This is to certify that the above mentioned candidate was a student of this institution. 
                   They have officially requested a transfer for the following reason: 
                ...
                <template v-else>
                   ...
                   moral conduct and social discipline throughout their academic session. 
                   They are a verified member of our alumni registry with zero disciplinary incidents.
                </template>
             </CertificateBase>
          </div>
       </div>
    ...
    <!-- Issuance Modal -->
    <AppModal v-model="showIssueModal" title="Issue Institutional Credential" size="md" class="print:hidden">
      <form @submit.prevent="handleIssue" class="space-y-6 p-2">
        <div class="space-y-2">
           ...
           <select v-model="form.student_id" class="w-full bg-gray-50 dark:bg-gray-900 border-none rounded-2xl p-4 font-black text-sm text-gray-900 dark:text-white outline-none">
              <option value="" disabled>Select Student...</option>
              <option v-for="s in studentStore.students" :key="s.id" :value="String(s.id)">{{ s.name }} ({{ s.class_name }})</option>
           </select>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6">
           <div class="space-y-2">
              ...
              <select v-model="form.type" class="w-full bg-gray-50 dark:bg-gray-900 border-none rounded-2xl p-4 font-black text-sm text-gray-900 dark:text-white outline-none">
      ...
      <template #footer>
        <div class="flex items-center justify-end gap-3 p-2">
          <AppButton variant="secondary" @click="showIssueModal = false" class="border-none font-black">Cancel</AppButton>
          <AppButton @click="handleIssue" class="px-10 rounded-2xl shadow-xl shadow-primary-200 uppercase text-[10px] font-black tracking-widest">Issue Credential</AppButton>
        </div>
      </template>
    </AppModal>
  </div>

<template>
  <div class="space-y-8 animate-fade-in-up">
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      <div v-for="stat in [
          { label: 'Total Revenue', val: feeStore.totalCollected, icon: '💰', color: 'text-green-600', bg: 'bg-green-50/50 dark:bg-green-900/20', isCurrency: true },
          { label: 'Outstanding Dues', val: feeStore.totalPending, icon: '⏳', color: 'text-amber-600', bg: 'bg-amber-50/50 dark:bg-amber-900/20', isCurrency: true },
          { label: 'Late Fees Collected', val: 4500, icon: '⚠️', color: 'text-red-500', bg: 'bg-red-50/50 dark:bg-red-900/20', isCurrency: true },
          { label: 'Recovery Rate', val: '92%', icon: '📈', color: 'text-blue-600', bg: 'bg-blue-50/50 dark:bg-blue-900/20' }
      ]" :key="stat.label" class="group relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-6 transition-all hover:-translate-y-1 hover:shadow-xl dark:border-gray-800 dark:bg-gray-800/50">
    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="inline-flex gap-1 rounded-2xl bg-gray-100 p-1.5 dark:bg-gray-800">
           <button
             ...
             :class="[
               ...
               activeTab === tab.key ? 'bg-white text-primary-600 shadow-md dark:bg-gray-700 dark:text-primary-400' : 'text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300',
             ]"
           >
             {{ tab.label }}
             <span v-if="tab.count" class="ml-2 rounded-full bg-primary-100 px-2 py-0.5 text-[9px] dark:bg-primary-900/40">{{ tab.count }}</span>
           </button>
        </div>
        <AppButton v-if="activeTab === 'payments'" @click="showCollectModal = true" class="shadow-xl shadow-primary-200">
          + Collect New Payment
        </AppButton>
        <AppButton v-if="activeTab === 'structures'" @click="showStructureModal = true" class="shadow-xl shadow-primary-200">
          + New Fee Structure
        </AppButton>
      </div>
      ...
      <transition name="page" mode="out-in">
        <div :key="activeTab">
          <!-- Payments Tab -->
          <AppCard v-if="activeTab === 'payments'" :no-padding="true" class="overflow-hidden border-none shadow-xl shadow-gray-200/50 dark:shadow-none">
              ...
              <table class="w-full text-left text-sm">
                <thead>
                  <tr class="border-y border-gray-50 bg-gray-50/30 dark:bg-gray-800/20 dark:border-gray-700/50">
                    <th class="px-8 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400">Payer Details</th>
                    <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400">Fee Category</th>
                    <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400 text-right">Invoice Bal</th>
                    <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400 text-right">Settled</th>
                    <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400 text-right">Pending</th>
                    <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400">Status</th>
                    <th class="px-8 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400 text-right">Actions</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                  <tr v-for="payment in feeStore.filteredPayments" :key="payment.id" class="group hover:bg-primary-50/30 dark:hover:bg-primary-900/10 transition-all">
                    <td class="px-8 py-5">
                      <div class="flex items-center gap-4">
                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gray-100 text-xs font-black dark:bg-gray-700">
                    ...
                    <td class="px-6 py-5">
                       <span class="rounded-lg bg-primary-50 px-2 py-1 text-[10px] font-black uppercase text-primary-600 dark:bg-primary-900/20">{{ payment.fee_name }}</span>
                    </td>
                    <td class="px-6 py-5 text-right font-black text-gray-900 dark:text-white">₹{{ payment.total_amount.toLocaleString('en-IN') }}</td>
                    <td class="px-6 py-5 text-right font-black text-green-600">₹{{ payment.paid_amount.toLocaleString('en-IN') }}</td>
                    <td class="px-6 py-5 text-right font-black" :class="payment.due_amount > 0 ? 'text-red-500' : 'text-gray-400'">
                      ₹{{ payment.due_amount.toLocaleString('en-IN') }}
                    </td>
                    <td class="px-6 py-5 border-l-4" :class="payment.due_amount > 0 ? 'border-amber-400' : 'border-green-400'">
                      <StatusBadge :color="statusColor(payment.status)">{{ payment.status }}</StatusBadge>
                    </td>
                    <td class="px-8 py-5 text-right">
                      <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                         <button v-if="payment.status !== 'paid'" @click="openQuickPay(payment)" class="rounded-xl p-2 text-primary-600 hover:bg-white dark:hover:bg-gray-800 shadow-sm border border-transparent hover:border-gray-100">
                           💰
                         </button>
                         <button v-if="payment.receipt_number" @click="viewReceipt(payment)" class="rounded-xl p-2 text-gray-400 hover:bg-white dark:hover:bg-gray-800 shadow-sm border border-transparent hover:border-gray-100">
                           📄
                         </button>
                      </div>
                    </td>
            ...
            <EmptyState v-if="!feeStore.filteredPayments.length" title="No matching entries" message="Try broader keywords or reset filters." />
          </AppCard>

          <!-- Due List Tab -->
          <AppCard v-if="activeTab === 'due'" title="Outstanding Due Ledger" :no-padding="true" class="overflow-hidden border-none shadow-xl shadow-gray-200/50 dark:shadow-none">
             <div class="overflow-x-auto">
               <table class="w-full text-left text-sm">
                 <thead>
                    <tr class="border-y border-gray-50 bg-red-50/20 dark:bg-red-900/10 dark:border-gray-700/50">
                      ...
                      <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400">Collection Group</th>
                      <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-red-500 text-right animate-pulse">Critical Dues</th>
                      <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400 text-right">Action Required</th>
                    </tr>
                 </thead>
                 <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                    <tr v-for="payment in feeStore.duePayments" :key="payment.id" class="hover:bg-red-50/30 dark:hover:bg-red-900/10 transition-all">
                      <td class="px-8 py-5">
                          <p class="font-bold text-gray-900 dark:text-white">{{ payment.student_name }}</p>
                          <p class="text-[10px] font-medium text-gray-400 uppercase tracking-tight">{{ payment.class_name }}</p>
                      </td>
                      <td class="px-6 py-5">
                         <span class="rounded-lg bg-gray-100 px-2 py-1 text-[10px] font-black uppercase text-gray-600 dark:bg-gray-700">{{ payment.fee_name }}</span>
                      </td>
                      <td class="px-6 py-5 text-right font-black text-red-500">₹{{ payment.due_amount.toLocaleString('en-IN') }}</td>
             ...
             <EmptyState v-if="!feeStore.duePayments.length" title="High Liquidity!" message="All outstanding dues have been recovered." />
          </AppCard>

          <!-- Fee Structures Tab -->
          <AppCard v-if="activeTab === 'structures'" :no-padding="true" class="overflow-hidden border-none shadow-xl shadow-gray-200/50 dark:shadow-none">
             <div class="overflow-x-auto">
               <table class="w-full text-left text-sm">
                 <thead>
                   <tr class="border-y border-gray-50 bg-gray-50/30 dark:bg-gray-800/20 dark:border-gray-700/50">
                     <th class="px-8 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400">Scheme Name</th>
                     <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400">Eligibility</th>
                     <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400 text-right">Standard Rate</th>
                     <th class="px-6 py-4 font-black uppercase tracking-widest text-[10px] text-gray-400">Deadline</th>
                   </tr>
                 </thead>
                 <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                   <tr v-for="s in feeStore.structures" :key="s.id" class="hover:bg-primary-50/30 dark:hover:bg-primary-900/10 transition-all">
                     ...
                     <td class="px-6 py-5 text-right font-black text-gray-900 dark:text-white">₹{{ s.amount.toLocaleString('en-IN') }}</td>
                     <td class="px-6 py-5">
    ...
    <AppModal v-model="showCollectModal" title="Recovery Console" size="md">
      <form @submit.prevent="handleCollect" class="space-y-6">
        <AppInput v-model="collectForm.studentSearch" label="Search Student Record" placeholder="Begin typing name..." :error="collectErrors.student" />
        
        <div v-if="collectForm.studentSearch && matchedDuePayments.length" class="space-y-2 rounded-2xl border border-gray-100 bg-gray-50 p-2 dark:border-gray-700 dark:bg-gray-800/50">
          <button
          ...
          >
            <div>
              ...
              <p class="text-[10px] font-bold text-primary-500 uppercase">{{ p.fee_name }} · Due: ₹{{ p.due_amount.toLocaleString('en-IN') }}</p>
            </div>
            <StatusBadge :color="p.status === 'partial' ? 'yellow' : 'red'">{{ p.status }}</StatusBadge>
          </button>
        </div>

        <div v-if="selectedPayment" class="rounded-2xl bg-gradient-to-br from-primary-600 to-primary-400 p-5 text-white shadow-lg shadow-primary-100">
          ...
          <div class="mt-2 flex items-center justify-between">
             ...
             <p class="text-xl font-bold bg-white/20 px-3 py-1 rounded-xl backdrop-blur-sm">₹{{ selectedPayment.due_amount.toLocaleString('en-IN') }}</p>
          </div>
          <p class="mt-1 text-xs font-medium opacity-90">{{ selectedPayment.fee_name }} for {{ selectedPayment.class_name }}</p>
        </div>

        <AppInput v-model="collectForm.amount" label="Settlement Amount (₹)" type="number" placeholder="Enter collection amount" required :error="collectErrors.amount" />
      ...
      <div v-if="quickPayTarget" class="space-y-6">
        <div class="rounded-2xl bg-gradient-to-br from-red-600 to-red-400 p-6 text-white shadow-xl shadow-red-100">
          ...
          <p class="mt-4 text-xs font-bold leading-relaxed opacity-90">
             Settling ledger for <span class="underline decoration-wavy">{{ quickPayTarget.student_name }}</span><br/>
             ({{ quickPayTarget.fee_name }})
          </p>
        </div>
        <AppInput v-model="quickPayAmount" label="Collection Value (₹)" type="number" :placeholder="`Max: ${quickPayTarget.due_amount}`" />
    ...
    <AppModal v-model="showReceiptModal" title="Payment Receipt" size="md">
      <div v-if="receiptData" class="space-y-6" id="receipt">
          ...
          <table class="w-full text-sm">
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
              <tr v-for="row in [
                ...
                { l: 'Amount Paid', v: '₹' + receiptData.paid_amount.toLocaleString('en-IN'), c: 'text-green-600 font-black' },
                { l: 'Balance Due', v: '₹' + receiptData.due_amount.toLocaleString('en-IN'), c: receiptData.due_amount > 0 ? 'text-red-600 font-black' : 'text-gray-400' }
              ]" :key="row.l">
        ...
        <div class="text-xs font-bold text-gray-400 text-center uppercase tracking-widest">
          Payment Method: <span class="text-gray-900 dark:text-white">{{ receiptData.payment_method?.replace('_', ' ') }}</span>
        </div>
      </div>
      <template #footer>
        <div class="flex items-center justify-end gap-3 w-full">
          <AppButton variant="secondary" @click="showReceiptModal = false" class="flex-1">Close</AppButton>
          <AppButton @click="handlePrintReceipt" class="flex-1 shadow-xl shadow-primary-200">Print Professional Receipt</AppButton>
        </div>
      </template>
    </AppModal>
    ...
    <Teleport to="body">
       <div v-if="isPrinting" id="receipt-print-engine" class="bg-white !p-0 !m-0 fixed inset-0 z-[9999] flex justify-center overflow-y-scroll print:overflow-visible">
          <div v-if="receiptData" class="py-10">
...
function handleCollect() {
  ...
  if (!selectedPayment.value) { collectErrors.student = 'Select student record'; return }
  const amt = Number(collectForm.amount)
  if (!amt || amt <= 0) { collectErrors.amount = 'Enter valid value'; return }
  if (amt > selectedPayment.value.due_amount) { collectErrors.amount = 'Exceeds balance'; return }
  ...
  if (settingsStore.settings.auto_sms_on_payment && selectedPayment.value) {
    ...
    smsStore.sendPaymentSms(phone, selectedPayment.value.student_name, amt, selectedPayment.value.fee_name, p?.receipt_number || '', p?.due_amount || 0)
  }
...
function handleQuickPay() {
  ...
  if (!amt || amt <= 0) { toast.warning('Enter valid value'); return }
  if (amt > quickPayTarget.value.due_amount) { toast.warning('Exceeds balance'); return }
  ...
  if (settingsStore.settings.auto_sms_on_payment) {
    ...
    smsStore.sendPaymentSms(phone, quickPayTarget.value.student_name, amt, quickPayTarget.value.fee_name, p?.receipt_number || '', p?.due_amount || 0)
  }
...
function handleAddStructure() {
  if (!structureForm.name || !structureForm.class_name || !structureForm.amount) {
    toast.warning('Fields required'); return
  }
  feeStore.addStructure({ name: structureForm.name, class_name: structureForm.class_name, amount: Number(structureForm.amount), due_date: structureForm.due_date, academic_year: structureForm.academic_year })

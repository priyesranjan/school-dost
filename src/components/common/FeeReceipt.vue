<template>
  <div class="receipt-container w-[210mm] bg-white p-16 shadow-2xl border-4 border-double border-gray-100 flex flex-col gap-10 relative overflow-hidden">
    ...
    <div class="grid grid-cols-2 gap-12 relative z-10">
      <div class="space-y-4">
        ...
        <div>
          ...
          <p class="text-sm font-mono font-black text-gray-700">STD/{{ 2025 + (payment.student_id % 10) }}/0{{ payment.student_id }}</p>
        </div>
      </div>
      <div class="space-y-4 text-right">
        ...
        <div>
          ...
          <span class="inline-flex items-center gap-2 rounded-lg bg-gray-100 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-gray-600">
      ...
      <table class="w-full text-left">
        <thead>
          <tr class="border-b-2 border-gray-900">
            <th class="py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Description of Fee Component</th>
            <th class="py-4 text-right text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Ledger Amount</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100 italic">
          <tr>
            <td class="py-6">
               ...
               <p class="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Academic Session 2025-26 · Term Phase 1</p>
            </td>
            <td class="py-6 text-right font-black text-gray-900 text-lg">₹{{ payment.total_amount.toLocaleString('en-IN') }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="mt-10 pt-10 border-t border-gray-100 grid grid-cols-2 gap-4 relative z-10">
       <div>
          <p class="text-[8px] font-bold text-gray-400 uppercase leading-relaxed max-w-[250px]">
             This is a digitally generated institutional receipt. No physical signature is required for official verification. 
             Security ID: {{ payment.id }}HASH-{{ payment.receipt_number }}
          </p>
       </div>
